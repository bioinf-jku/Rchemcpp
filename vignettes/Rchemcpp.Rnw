...
The \Rchemcpp\ package implements functions that compute similarities between
small drug-like molecules. Similarity measures for compounds are highly relevant
...
asked to cite it as follows: \citep{Mahr2012}.
